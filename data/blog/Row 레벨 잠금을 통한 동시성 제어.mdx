...
0 ~ 10 사이의 임의의 offset을 적용하게 된다면 최대 10개의 가상번호를 동시에 할당하는 것처럼 처리할 수 있다.
...
WHERE rn IS NULL
LIMIT 1 
OFFSET 7 -- offset에 의해 서로 다른 행에 접근한다. 이로 인해 동시성이 향상된다. 
FOR UPDATE
...
WHERE rn IS NULL
LIMIT 1 
OFFSET 0 -- offset에 의해 서로 다른 행에 접근한다. 이로 인해 동시성이 향상된다. 
FOR UPDATE
```